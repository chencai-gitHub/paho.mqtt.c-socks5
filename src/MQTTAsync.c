		...
		int persistence_type, void* persistence_context,  MQTTAsync_createOptions* options)
{
	...
	*handle = m;
	...
	m->c->context = m;
	m->c->mqttAsync_tostop = &MQTTAsync_tostop;
	m->c->outboundMsgs = ListInitialize();
	m->c->inboundMsgs = ListInitialize();
	m->c->messageQueue = ListInitialize();
	m->c->clientID = MQTTStrdup(clientId);
	m->c->serverURI = MQTTStrdup(serverURI);
...
int MQTTAsync_connect(MQTTAsync handle, const MQTTAsync_connectOptions* options)
{
	...
	{
		m->c->socks5_state_mutex = PTHREAD_MUTEX_INITIALIZER;
		m->c->out_packet_mutex = PTHREAD_MUTEX_INITIALIZER;
		m->c->socks5_host = MQTTStrdup(options->socks5_host);
		m->c->net.socks5_host = MQTTStrdup(options->socks5_host);
			
		m->c->socks5_port = options->socks5_port;
		m->c->net.socks5_port = options->socks5_port;
			
		if (options->socks5_username)
		{
			m->c->socks5_username = MQTTStrdup(options->socks5_username);
			m->c->net.socks5_username = MQTTStrdup(options->socks5_username);
		}
			
		if (options->socks5_password)
		{
			m->c->socks5_password = MQTTStrdup(options->socks5_password);
			m->c->net.socks5_password = MQTTStrdup(options->socks5_password);
		}
	}
