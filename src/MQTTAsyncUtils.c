...
//static int MQTTAsync_socketLost(int* socket);
...
static int MQTTAsync_connecting(MQTTAsyncs* m)
{
	...
	if (m->c->connect_state == TCP_IN_PROGRESS) /* TCP connect started - check for completion */
	{
		...
		else
		{
			...
			else if (m->c->net.socks5_host)
			{
				m->c->connect_state = SOCKS5_PROXY_CONNECT_IN_PROGRESS;
				if ((rc = WebSocket_socks5_proxy_connect(m->c)) == SOCKET_ERROR)
					goto exit;
	...
	else if (m->c->connect_state == SOCKS5_PROXY_CONNECT_IN_PROGRESS)
	{
		enum pahomqtt_client_socks5_state socks5State = pahomqtt_get_socks5_state(m->c);
		if (socks5State == pahomqtt_cs_new)
		{
			m->c->connect_state = WAIT_FOR_CONNACK; /* Websocket upgrade completed, in which case send the MQTT connect packet */
			if ((rc = MQTTPacket_send_connect(m->c, m->connect.details.conn.MQTTVersion, m->connectProps, m->willProps)) == SOCKET_ERROR)
				goto exit;
		}
		else if (socks5State == pahomqtt_cs_error)
		{
			rc = SOCKET_ERROR;
			goto exit;
		}
...
exit:
	if ((rc != 0 && rc != TCPSOCKET_INTERRUPTED && (m->c->connect_state != SSL_IN_PROGRESS && m->c->connect_state != WEBSOCKET_IN_PROGRESS && rc != SOCKS5_PROXY_CONNECT_IN_PROGRESS)) || (rc == SSL_FATAL))
	{
...
static int MQTTAsync_socketLost(int* socket)
{
	...
	int fdcount = select(0, &readFdset, NULL, NULL, NULL);
	if (fdcount != -1)
	{
		char buf[10];
		int len = recv(socket, buf, 1, MSG_PEEK);

		char rce1[10], error1[10];
		itoa(fdcount, rce1, 10);
		itoa(len, error1, 10);
		WriteLogFile(LOG_DEBUG, "---- test fdcount = ---------");
		WriteLogFile(LOG_DEBUG, rce1);
		WriteLogFile(LOG_DEBUG, "---- test recv len = ---------");
		WriteLogFile(LOG_DEBUG, error1);

		if (fdcount == 1 && len == 0)
		{
			WriteLogFile(LOG_DEBUG, "----------- socket lost 1111111 ---------------");
			rc = 0;
		}
		if (len <= 0)
		{
			...
			{
				WriteLogFile(LOG_DEBUG, "----------- socket ok ---------------");
				rc = 1;
			}
			else
			{
				...
				{
				case 0:
					WriteLogFile(LOG_DEBUG, "----------- socket proxy error  ---------------");
					rc = 0;
				case COMPAT_ECONNRESET:
					WriteLogFile(LOG_DEBUG, "----------- socket lost 222222 ---------------");
					rc = 0;
				default:
					WriteLogFile(LOG_DEBUG, "----------- socket error  ---------------");
					rc = 0;
				}
			}
		}
	...
	{
		WriteLogFile(LOG_DEBUG, "----------- select failed  ---------------");
		rc = 1;
	}
	FD_ZERO(&readFdset); //清空文件描述符集合
	return rc;
