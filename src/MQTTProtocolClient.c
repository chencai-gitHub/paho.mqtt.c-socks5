...
void MQTTProtocol_freeClient(Clients* client)
{
	...
	client->httpProxy = NULL;
	if (client->httpsProxy)
		free(client->httpsProxy);
	client->httpsProxy = NULL;
	if (client->net.http_proxy_auth)
		free(client->net.http_proxy_auth);
	client->net.http_proxy_auth = NULL;

	//socks5
	if (client->socks5_host)
		free(client->socks5_host);
	client->socks5_host = NULL;
	if (client->socks5_username)
		free(client->socks5_username);
	client->socks5_username = NULL;
	if (client->socks5_password)
		free(client->socks5_password);
	client->socks5_password = NULL;

	if (client->net.socks5_password)
		free(client->net.socks5_password);
	client->net.socks5_password = NULL;
	if (client->net.socks5_username)
		free(client->net.socks5_username);
	client->net.socks5_username = NULL;
	if (client->net.socks5_password)
		free(client->net.socks5_password);
	client->net.socks5_password = NULL;

	if (client->serverURI)
		free(client->serverURI);
	client->serverURI = NULL;

	if(client->socks5_state_mutex)
		pthread_mutex_destroy(&client->socks5_state_mutex);
	client->socks5_state_mutex = NULL;

	if (client->out_packet_mutex)
		pthread_mutex_destroy(&client->out_packet_mutex);
	client->out_packet_mutex = NULL;

	if (client->out_packet)
		free(client->out_packet);
	client->out_packet = NULL;

	if (client->out_packet_last)
		free(client->out_packet_last);
