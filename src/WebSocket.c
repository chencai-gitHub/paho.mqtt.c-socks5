...
int WebSocket_socks5_proxy_connect(Clients *aClient)
{
	//int port, i, rc = 0, buf_len = 0;
	//char *buf = NULL;
	//size_t hostname_len, actual_len = 0;
	...
	while (1)
	{
		...
		enum pahomqtt_client_socks5_state currentState;
		currentState = pahomqtt_get_socks5_state(aClient);
		if (currentState == pahomqtt_cs_new)
		{
			rc = 0;
			break;
		}
		else if (currentState == pahomqtt_cs_error)
		{
			rc = SOCKET_ERROR;
		}
		else
