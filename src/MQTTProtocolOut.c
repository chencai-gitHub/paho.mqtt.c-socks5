...
#endif
{
	...
	else if (aClient->net.socks5_host)
	{
		addr_len = MQTTProtocol_addressPort(aClient->net.socks5_host, &port, NULL, aClient->net.socks5_port);
		rc = Socket_new(aClient->net.socks5_host, addr_len, port, &(aClient->net.socket));
	}
#if defined(OPENSSL)
	...
	{	/* TCP connect completed. If SSL, send SSL connect */
		...
		if (aClient->net.socks5_host)
		{
			aClient->connect_state = SOCKS5_PROXY_CONNECT_IN_PROGRESS;
			rc = WebSocket_socks5_proxy_connect(aClient);
		}
